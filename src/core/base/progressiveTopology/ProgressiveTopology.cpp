...
void ttk::ProgressiveTopology::updateLinkPolarityPonctual(
  std::vector<std::pair<polarity, polarity>> &vlp) {
...
void ttk::ProgressiveTopology::copyPolarity(
  std::vector<std::pair<polarity, polarity>> &vlp,
  std::vector<std::pair<polarity, polarity>> &vlpTarget) {
  vlpTarget.resize(vlp.size());
  for(size_t i = 0; i < vlp.size(); i++) {
    vlpTarget[i] = vlp[i];
  }
}

void ttk::ProgressiveTopology::copyPolarityAndFlagChanges(
  std::vector<std::pair<polarity, polarity>> &vlp,
  std::vector<std::pair<polarity, polarity>> &vlpTarget) {
  vlpTarget.resize(vlp.size());
  for(size_t i = 0; i < vlp.size(); i++) {
    if(vlpTarget[i] != vlp[i]) {
      vlpTarget[i].second = 255;
    }
  }
}

void ttk::ProgressiveTopology::resetCriticalType(
  SimplexId vertexId,
  DynamicTree &link,
  std::vector<std::pair<polarity, polarity>> &vlp,
  std::vector<std::pair<SimplexId, SimplexId>> &vl) const {
