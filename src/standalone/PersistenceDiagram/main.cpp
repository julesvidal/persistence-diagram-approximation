...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    parser.setArgument("B", &backEnd, "Method (0:FTM, 1: Hierarchical)", true);
    parser.setArgument("S", &startingRL,
                       "Starting Resolution Level for the hierarchy "
                       "(-1: finest level)",
                       true);
    parser.setArgument("E", &stoppingRL,
                       "Stopping Resolution Level for for the hierarchy "
                        "(-1: finest level)",
                       true);
    parser.setArgument("T", &tl, "Time limit for progressive method", true);
    parser.setArgument("A", &useAdaptive, "Use Approximate approach", true);
    parser.setArgument("e", &eps, "Epsilon", true);
    /* parser.setArgument("b", &nbBuckets, "nb buckets", true); */
  ...
  persistenceDiagram->SetEpsilon(eps);
  persistenceDiagram->SetUseAdaptive(useAdaptive);
  persistenceDiagram->SetNbBuckets(nbBuckets);
